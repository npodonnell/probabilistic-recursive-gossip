...
The ideal peers of a node are the nodes whose IDs are exactly $2^i$  away from it where $i \in \{0 \cdots N – 1\}$, modulo $2N$. So if a node’s id is $x$, its ideal peers are:
